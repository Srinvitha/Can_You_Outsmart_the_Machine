...
function generatePattern(difficulty){const pool=PATTERN_FAMILIES.filter(f=>f.difficulty===difficulty),family=pool[rint(0,pool.length-1)];let pattern;do{pattern=family.gen()}while(pattern.values.some(value=>!Number.isSafeInteger(value)||Math.abs(value)>999999)||new Set(pattern.values).size<5);pattern.difficulty=difficulty;pattern.level=patternDifficultyLabel(difficulty);pattern.familyName=family.name;return pattern}
function patternRoundDifficulty(){const q=state.roundQ,wins=state.roundWins;if(q>=4)return 4;if(q===3)return wins>=2?3:2;if(q===2)return wins>=1?2:1;return 1}
function startPattern(){if(state.tick)clearInterval(state.tick);if(state.machineTimer)clearTimeout(state.machineTimer);state={roundQ:1,roundTotal:5,roundWins:0,over:false};startPatternQuestion()}
function startPatternQuestion(){
 if(state.tick)clearInterval(state.tick);if(state.machineTimer)clearTimeout(state.machineTimer);
 const difficulty=patternRoundDifficulty(),pattern=generatePattern(difficulty),limit=difficulty===4?10:difficulty===3?12:14;
 state.p=pattern;state.answer=pattern.values.slice(-2);state.submitted=false;state.machineLocked=false;state.t0=Date.now();state.limit=limit;state.machineTime=patternMachineTime(difficulty);
 const shown=pattern.values.slice(0,7);
 header('🧩 Pattern Duel','Predict the next TWO terms. The duel gets harder as you prove yourself.',`<div class="pattern-battle-head"><div class="pattern-level ${difficulty}">${pattern.level} • ${pattern.familyName} • Q${state.roundQ}/${state.roundTotal}</div><div class="pattern-machine">🤖 MACHINE <span id="patternMachineState">ANALYZING</span></div></div><div class="pattern-sequence">${shown.map((value,index)=>`<span>${value}</span>${index<shown.length-1?'<i>→</i>':''}`).join('')}<i>→</i><strong>?</strong><i>→</i><strong>?</strong></div><div class="pattern-inputs"><input id="pat1" type="number" inputmode="numeric" placeholder="Next term"><span>→</span><input id="pat2" type="number" inputmode="numeric" placeholder="Next term"></div><div class="pattern-timer"><span>TIME</span><b id="patternTimer">${limit.toFixed(1)}</b><div class="progress"><div class="bar" id="patternBar"></div></div></div><div class="pattern-duel-note"><b>Round score: ${state.roundWins}/${state.roundQ-1}</b> • Q4 and Q5 are hard.</div>${btns([B('⚔️ BEAT THE MACHINE','checkPattern'),B('🧠 Show the math','patternMath','secondary'),B('↻ Restart round','startPattern','secondary')])}<div id="msg" class="notice">Find the rule before the machine locks in.</div>`);
...
function patternMachineLock(timeout=false){if(state.over||state.submitted)return;state.machineLocked=true;const machineState=document.getElementById('patternMachineState');if(machineState)machineState.textContent='LOCKED IN';if(timeout){state.submitted=true;clearInterval(state.tick);notice('🤖 <b>MACHINE WINS THIS QUESTION.</b> It locked in before you.','bad');setTimeout(nextPatternQuestion,700)}}
function nextPatternQuestion(){if(state.roundQ>=state.roundTotal){finishPatternRound();return}state.roundQ++;startPatternQuestion()}
function finishPatternRound(){state.over=true;clearInterval(state.tick);if(state.machineTimer)clearTimeout(state.machineTimer);if(state.roundWins>=3){notice(`🏆 <b>MACHINE OUTSMARTED.</b> You beat it on <b>${state.roundWins}/${state.roundTotal}</b> duels.`,'good');saveWin('You beat Pattern Duel',1+(state.roundWins-3)*.15)}else notice(`🤖 <b>MACHINE WINS THE ROUND.</b> You beat it on <b>${state.roundWins}/${state.roundTotal}</b>.`,'bad')}
function checkPattern(){if(state.over||state.submitted)return;const first=Number(document.getElementById('pat1').value),second=Number(document.getElementById('pat2').value);if(!Number.isFinite(first)||!Number.isFinite(second)){notice('Enter both predicted terms.','warn');return}state.submitted=true;clearInterval(state.tick);if(state.machineTimer)clearTimeout(state.machineTimer);const elapsed=(Date.now()-state.t0)/1000,correct=first===state.answer[0]&&second===state.answer[1],machineBeaten=elapsed*1000<state.machineTime;if(correct&&machineBeaten){state.roundWins++;notice(`🏆 <b>MACHINE OUTSMARTED.</b> ${first}, ${second} in <b>${elapsed.toFixed(2)}s</b>.`,'good')}else if(correct)notice(`⚔️ <b>RIGHT, BUT TOO SLOW.</b> The machine locked in at ${(state.machineTime/1000).toFixed(2)}s.` ,'warn');else notice(`🤖 <b>MACHINE WINS THIS QUESTION.</b> Correct terms: <b>${state.answer[0]}, ${state.answer[1]}</b>.`,'bad');setTimeout(nextPatternQuestion,850)}
...
function symbioticWin(a,b,maxA,maxB,memo=new Map()){const key=`${a},${b},${maxA},${maxB}`;if(memo.has(key))return memo.get(key);if(a===0&&b===0)return false;for(const move of [{pile:'a',max:maxA,count:a},{pile:'b',max:maxB,count:b}])for(let amount=1;amount<=Math.min(move.max,move.count);amount++){const nextA=move.pile==='a'?a-amount:a,nextB=move.pile==='b'?b-amount:b;if(nextA===0&&nextB===0){memo.set(key,true);return true}const nextMaxA=move.pile==='b'?4-amount:3,nextMaxB=move.pile==='a'?4-amount:3;if(!symbioticWin(nextA,nextB,nextMaxA,nextMaxB,memo)){memo.set(key,true);return true}}memo.set(key,false);return false}
function symbioticBestMove(){const memo=state.memo||(state.memo=new Map());for(const move of symbioticMoves()){const nextA=state.a-(move.pile==='a'?move.amount:0),nextB=state.b-(move.pile==='b'?move.amount:0);if(nextA===0&&nextB===0)return move;const maxA=move.pile==='b'?4-move.amount:3,maxB=move.pile==='a'?4-move.amount:3;if(!symbioticWin(nextA,nextB,maxA,maxB,memo))return move}return null}
function symbioticAI(){if(state.over||state.turn!=='ai')return;const best=symbioticBestMove();if(!best){state.over=true;notice('🏆 <b>YOU WIN.</b> The machine has no winning response.','good');saveWin('You beat Symbiotic Feedback',1);return}applySymbioticMove(best.pile,best.amount,'ai');if(state.over)return;state.turn='human';renderSymbiotic();notice(`🤖 Machine removed <b>${best.amount}</b> from pile ${best.pile.toUpperCase()}. Your turn.`)}
...
function parityWin(a,b,lastStyle,memo=new Map()){const key=`${a},${b},${lastStyle||'ANY'}`;if(memo.has(key))return memo.get(key);if(a===0&&b===0)return false;const required=lastStyle==='SINGLE'?'BOTH':lastStyle==='BOTH'?'SINGLE':'ANY';const moves=[];if(required==='SINGLE'||required==='ANY'){for(let k=1;k<=a;k++)moves.push({type:'A',amount:k});for(let k=1;k<=b;k++)moves.push({type:'B',amount:k})}if(required==='BOTH'||required==='ANY')for(let k=1;k<=Math.min(a,b);k++)moves.push({type:'BOTH',amount:k});for(const move of moves){const nextA=a-(move.type==='A'||move.type==='BOTH'?move.amount:0),nextB=b-(move.type==='B'||move.type==='BOTH'?move.amount:0);if(nextA===0&&nextB===0){memo.set(key,true);return true}if(!parityWin(nextA,nextB,move.type==='BOTH'?'BOTH':'SINGLE',memo)){memo.set(key,true);return true}}memo.set(key,false);return false}
function parityBestMove(){const memo=state.memo||(state.memo=new Map());for(const move of parityMoves()){const nextA=state.a-(move.type==='A'||move.type==='BOTH'?move.amount:0),nextB=state.b-(move.type==='B'||move.type==='BOTH'?move.amount:0),style=move.type==='BOTH'?'BOTH':'SINGLE';if(nextA===0&&nextB===0)return move;if(!parityWin(nextA,nextB,style,memo))return move}return null}
function parityAI(){if(state.over||state.turn!=='ai')return;const best=parityBestMove();if(!best){state.over=true;notice('🏆 <b>YOU WIN.</b> The exact parity solver found no winning response.','good');saveWin('You locked Parity-Shift Wythoff',1);return}applyParityMove(best.type,best.amount,'ai');if(state.over)return;state.turn='human';renderParity();notice(`🤖 Machine used <b>${best.type==='BOTH'?'Both':best.type} − ${best.amount}</b>. Your turn.`)}
